...
export function SceneMenu({ sceneCollection, setSceneCollection, sceneIDMap, setSceneIDMap, linkFactory,
    ...
    useEffect(() => {
        let myCollection = sceneCollection;
        let collectionJSX = [];
        ...
        if (collectionUpdate && myCollection.length > 0) {
            for (let i = 0; i < myCollection.length; i++) {
                ...
                if (myCollection[i][0] == '') {
                    if (currentCanvasID == myCollection[i][1][0].getID()) {
                        collectionJSX.push(<div className={'SceneEl Scene_' + myCollection[i][1][0].getID()} key={i}>
                            <h3>{myCollection[i][1][0].getName()}</h3>
                            <IconButton
                            ...
                            <Button bgColor={'rgba(95, 159, 255, 1)'} _hover={{ bg: 'rgba(130, 180, 255, 1)' }} _focus={{bg: 'rgba(130, 180, 255, 1)' }}
                                onClick={() => {
                                    alert('Scene ' + myCollection[i][1][0].getName() + ' is already displayed in GM mode');
                                }}>GM</Button>
                        ...
                        collectionJSX.push(<div className={'SceneEl Scene_' + myCollection[i][1][0].getID()} key={i}>
                            ...
                            <Button onClick={() => {
                                ...
                                let newCanvas = document.getElementById('scene_' + myCollection[i][1][0].getID());
                                if(newCanvas && prevCanvas)
                                {
                                    ...
                                    setCurrentCanvasID(myCollection[i][1][0].getID());
                                    setCanvas(canvasCollection[i][1][0]);
                                    setCurrentScene(myCollection[i][1][0]);
                    ...
                    let groupJSX = [];
                    for (let j = 0; j < myCollection[i][1].length; j++) {
                        if (currentCanvasID == myCollection[i][1][j].getID()) {
                            groupJSX.push(<div className={'SceneEl Scene_' + myCollection[i][1][j].getID()}>
                                <h3>{myCollection[i][1][j].getName()}</h3>
                                <IconButton
                                    className='RenameButton'>
                                    <Center className='RenameIcon'>
                                        <FaEdit className='RenameIcon' />
    ...
    return (
        <div>
            <div className='ButtonRow'>
                ...
                <BattleMapCreationMenu linkFactory={linkFactory} sceneCollection={sceneCollection}
